...
namespace VoteService.Service
{
    public class VoteDbService : IVoteDbService
    {
        ...
        public async Task EntryVoteProcessAsync(EntryVoteCommand command)
        {
            ...
            {
                var entity = connection.Query<object>("SELECT * FROM EntryVotes WHERE EntryId = @EntryId and UserId = @UserId",
                                        new { EntryId = command.EntryId, UserId = command.UserId }).ToList();
                if (entity is not null && !entity.Any())
                    await connection.ExecuteAsync("INSERT INTO EntryVotes (EntryId,UserId,Type) VALUES (@EntryId,@UserId,@Type)",
                    new { EntryId = command.EntryId, UserId = command.UserId, Type = command.Type });
                else
                    await connection.ExecuteAsync("UPDATE EntryVotes SET Type=@Type WHERE EntryId = @EntryId and UserId = @UserId",
                    new { EntryId = command.EntryId, UserId = command.UserId, Type = command.Type });
            }
        }
    }
