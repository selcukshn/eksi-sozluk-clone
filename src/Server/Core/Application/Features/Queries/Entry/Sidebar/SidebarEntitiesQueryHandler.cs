...
{
    public class SidebarEntitiesQueryHandler : GenericHandler<IEntryRepository, SidebarEntriesQuery, List<SidebarViewModel>>
    {
        public SidebarEntitiesQueryHandler(IEntryRepository repository, IMapper mapper) : base(repository, mapper) { }

        public override async Task<List<SidebarViewModel>> Handle(SidebarEntriesQuery request, CancellationToken cancellationToken)
        {
